    ...
    <div>
        <div style="width: 40%">
            <el-form ref="form"  label-width="40px">
                ...
                <el-form-item>
                    <el-button type="primary" @click="establish()">立即修改</el-button>
...
<script>
    export default {
        ...
        mounted: function () {
            ...
            this.$http.get('/modify_nav?id='+this.id).then(res=> {    //字符串拼接，this.id要放在引号外才能解析
                if (res.data.code == 1) {
        ...
        methods: {
            establish(){
                ...
                this.$http.post('/modify_nav_',parse).then(res=>{
                    if(res.data.code==1){
                        alert('修改成功');
